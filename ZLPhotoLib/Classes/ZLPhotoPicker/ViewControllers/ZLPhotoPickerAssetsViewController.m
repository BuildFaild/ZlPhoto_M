...
static NSInteger addImage = 0;
static NSInteger saveImageIndex = 0;
...
static ZLCameraViewController *_cameraVc;
- (void)pickerCollectionViewDidCameraSelect:(ZLPhotoPickerCollectionView *)pickerCollectionView{
    ...
    __weak typeof(self)weakSelf = self;
    _cameraVc.maxCount = self.maxCount;
    _cameraVc.callback = ^(NSArray *camera){
...
- (void)saveImages:(NSArray *)images {
    ...
    for (NSInteger i = 0; i < images.count; i++) {
        ZLCamera *Asset = images[i];
        
        UIImage *image = [UIImage imageWithContentsOfFile:Asset.imagePath];
...
- (void)image: (UIImage *) image didFinishSavingWithError: (NSError *) error contextInfo: (void *) contextInfo {
    saveImageIndex++;
    if (saveImageIndex == addImage) {
        NSLog(@"保存完了");

        [_cameraVc cancel:nil];
    }
    addImage = 0;
    saveImageIndex = 0;
}
